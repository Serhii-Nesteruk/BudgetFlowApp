...
public class UserSettingsController : ControllerBase
{
    ...
    public UserSettingsController(IUserService userService, IConfiguration configuration)
    {
    ...
    [Authorize]
    [HttpPost("telegram/connection-code")]
    public async Task<IActionResult> GenerateTelegramCode()
    {
        var link = _configuration.GetValue<string>("Telegram:BotLink") ?? "https://t.me/finance_tracker_demo_bot";
        return Ok(await _userService.GenerateTelegramConnectionCodeAsync(User.GetUserId(), link));
    }
    [AllowAnonymous]
    [HttpPost("telegram/verify")]
    public async Task<IActionResult> VerifyTelegramCode([FromBody] TelegramVerifyCodeDto dto)
    {
        try
        {
            var account = await _userService.VerifyTelegramConnectionCodeAsync(dto);
            return account == null ? BadRequest("Invalid or expired code.") : Ok(account);
        }
