...
{
    private readonly IBudgetService _service; public BudgetsController(IBudgetService service)
    {
        _service = service;
    }
    ...
    {
        var x = await _service.GetByIdForVisibleUserIdAsync(id, User.GetUserId());
        return x == null ? NotFound() : Ok(x.ToDto());
    }
    [HttpGet("shared/{token:guid}")]
    public async Task<IActionResult> GetBySharedLink(Guid token)
    {
        var x = await _service.GetByShareTokenForVisibleUserIdAsync(token, User.GetUserId());
        return x == null ? Forbid() : Ok(x.ToDto());
    }
    [HttpPost]
    public async Task<IActionResult> Create([FromBody] BudgetDto dto)
    {
        try
        {
            var x = await _service.AddAsync(dto, User.GetUserId());
    ...
    public async Task<IActionResult> Update(int id, [FromBody] BudgetDto dto)
    {
        try
        {
            var x = await _service.UpdateAsync(id, dto, User.GetUserId());
            return x == null ? NotFound() : Ok(x.ToDto());
        }
        catch (ArgumentException e) { return BadRequest(e.Message); }
    }
    ...
    [HttpPost("{id:int}/shared-users")]
    public async Task<IActionResult> Share(int id, [FromBody] ShareBudgetUserDto dto)
    {
        try
        {
            return Result(await _service.ShareWithUserAsync(id, dto.Email, User.GetUserId()));
        }
        catch (ArgumentException e) { return BadRequest(e.Message); }
    }
    ...
    [HttpPost("{id:int}/plan-next-months")]
    public async Task<IActionResult> PlanNextMonths(int id, [FromBody] BudgetPlanNextMonthsDto dto)
    {
        try
        {
            var items = await _service.PlanNextMonthsAsync(id, dto.Months, User.GetUserId());
