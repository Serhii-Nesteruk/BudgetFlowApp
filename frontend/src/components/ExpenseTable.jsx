...
export default function ExpenseTable({
  ...
  const arrow = (col) => {
    ...
    return (
      <span className={[styles.sa, styles.sorted].join(" ")}>{sortDir === 1 ? "↓" : "↑"}</span>
    );
  };
      ...
      <div className={[styles.tableCard, styles.desktopOnly].join(" ")}>
            ...
            {!page.length ? (
              ...
              page.map((entry) => {
                ...
                return [
                  ...
                  >
                    <td>
                      <span className={[styles.arrow, isExp ? styles.arrowOpen : ""].join(" ")}>
                        ▶
                      </span>
                    </td>
                    <td>{fmtDate(entry.date)}</td>
                    <td className={styles.amount}>
                      −{ttl.toFixed(2).replace(/\.00$/, "")} {symbol}
                    </td>
                    <td>
                      {places.map((p) => (
                        <Tag key={p}>{p}</Tag>
                    ...
                    <td>
                      <span className={styles.detailSummary}>{detail}</span>
                    </td>
                    <td onClick={(e) => e.stopPropagation()} style={{ whiteSpace: "nowrap" }}>
                      <div className={styles.rowActions}>
                        ...
                        >
                          <IconEdit />
                          <span className={styles.btnLabel}>Редагувати</span>
                        </button>
                        <button
                          className={[styles.btnSm, styles.btnSmDanger].join(" ")}
                          onClick={() => onDelete(entry.id)}
                          title="Видалити"
                        >
                          <IconTrash />
                          <span className={styles.btnLabel}>Видалити</span>
                        </button>
                      </div>
                    </td>
                  </tr>,
        ...
        <div className={styles.pager}>
          ...
          <div className={styles.pagerButtons}>
            ...
            {pageNums.map((n, i) =>
              n === "…" ? (
                <span key={i} style={{ padding: "4px 2px", color: "var(--muted)", fontSize: 12 }}>
                  …
                </span>
              ) : (
                <button
      ...
      <div className={[styles.mobileList, styles.mobileOnly].join(" ")}>
        {!page.length ? (
          ...
          page.map((entry) => {
            ...
            return (
              <div
                key={entry.id}
                className={[styles.mobileCard, isExp ? styles.mobileCardExpanded : ""].join(" ")}
              >
                <div className={styles.mobileCardMain} onClick={() => onToggleExpand(entry.id)}>
                  ...
                  <div className={styles.mobileCardRight}>
                    ...
                    <div
                      className={[styles.mobileChevron, isExp ? styles.mobileChevronOpen : ""].join(
                        " "
                      )}
                    >
                      <IconChevron />
                ...
                {isExp && (
                  <div className={styles.mobileExpanded}>
                    ...
                    <div className={styles.mobileActions}>
                      ...
                      <button
                        className={[styles.mobileBtn, styles.mobileBtnDanger].join(" ")}
                        onClick={() => onDelete(entry.id)}
                      >
                        <IconTrash /> Видалити
                      </button>
        ...
        {pageCount > 1 && (
          <div className={styles.mobilePager}>
            ...
            <span className={styles.mobilePagerInfo}>
              {curPage} / {pageCount}
            </span>
            <button
              className={styles.mobilePagerBtn}
