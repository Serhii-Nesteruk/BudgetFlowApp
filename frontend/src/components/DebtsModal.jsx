...
export default function DebtModal({
  ...
  function validate() {
    ...
    if (!form.creditor.trim())
      e.creditor = isReceivable ? "Вкажіть, хто винен" : "Вкажіть кредитора";
    if (!form.amount || isNaN(Number(form.amount)) || Number(form.amount) <= 0)
      e.amount = "Вкажіть суму";
    if (!form.dueDate) e.dueDate = "Вкажіть дату";
    if (form.type === "installment") {
      if (!form.totalInstallments || isNaN(Number(form.totalInstallments)))
        e.totalInstallments = "Вкажіть кількість";
      if (!form.monthlyPayment || isNaN(Number(form.monthlyPayment)))
        e.monthlyPayment = "Вкажіть суму платежу";
      ...
      <div className={styles.modal}>
        <div className={styles.header}>
          <h2 className={styles.title}>
            {debt ? "Редагувати запис" : isReceivable ? "Записати позику" : "Новий борг"}
          </h2>
        ...
        <div className={styles.body}>
          <div className={styles.directionGrid}>
            {DIRECTIONS.map((item) => (
              <button
                ...
                className={[
                  styles.directionBtn,
                  form.direction === item.value ? styles.directionBtnActive : "",
                ].join(" ")}
          ...
          <div className={styles.row}>
            <label className={styles.label}>
              {isReceivable ? "Хто винен мені *" : "Кому винен *"}
            </label>
            <input
          ...
          <div className={styles.cols2}>
            <div className={styles.row}>
              ...
              <input
                className={[styles.input, errors.amount ? styles.inputErr : ""].join(" ")}
                type="number"
                min="0"
                step="0.01"
                value={form.amount}
            ...
            <div className={styles.row}>
              ...
              <input
                className={[styles.input, errors.dueDate ? styles.inputErr : ""].join(" ")}
                type="date"
                value={form.dueDate}
          ...
          {form.type === "installment" && (
            <div className={styles.cols2}>
              <div className={styles.row}>
                ...
                <input
                  className={[styles.input, errors.totalInstallments ? styles.inputErr : ""].join(
                    " "
                  )}
                  type="number"
                  min="1"
                ...
                {errors.totalInstallments && (
                  <span className={styles.err}>{errors.totalInstallments}</span>
                )}
              </div>
              <div className={styles.row}>
                <label className={styles.label}>Щомісячний платіж *</label>
                <input
                  className={[styles.input, errors.monthlyPayment ? styles.inputErr : ""].join(" ")}
                  type="number"
                  min="0"
                ...
                {errors.monthlyPayment && (
                  <span className={styles.err}>{errors.monthlyPayment}</span>
                )}
              </div>
              <div className={styles.row}>
                <label className={styles.label}>Дата початку *</label>
                <input
                  className={[styles.input, errors.startDate ? styles.inputErr : ""].join(" ")}
                  type="date"
            ...
            <div className={styles.priorityRow}>
              {PRIORITIES.map((p) => (
                <button
                  key={p}
                  type="button"
                  className={[styles.priBtn, form.priority === p ? styles.priBtnActive : ""].join(
                    " "
                  )}
        ...
        <div className={styles.footer}>
          ...
          <button className={styles.btnSave} onClick={handleSubmit}>
            {debt ? "Зберегти" : isReceivable ? "Записати позику" : "Додати борг"}
          </button>
        </div>
      </div>
    </div>,
