    ...
    <div className={[
        ...
        <div className={styles.leftGroup}>
          ...
          <div className={styles.meta}>
            ...
            <div className={styles.tags}>
              <span className={[styles.directionBadge, isReceivable ? styles.directionBadgeReceivable : ""].join(" ")}>
                {isReceivable ? "Вам винні" : "Ви винні"}
              </span>
              <span className={[styles.badge, styles[`badge_${sc.cls}`]].join(" ")}>{sc.label}</span>
              <span className={styles.priDot} style={{ background: PRI_COLOR[debt.priority] }} title={`Пріоритет ${debt.priority}`} />
            </div>
          </div>
        </div>
      ...
      {(debt.type === "installment" || debt.status === "partial") && debt.amount > 0 && (
        <div className={styles.progressWrap}>
          ...
          <span className={styles.progressLabel}>{Math.round(progress)}% {isReceivable ? "повернуто" : "сплачено"}</span>
        </div>
      )}
      ...
      <div className={styles.dueRow}>
        <span className={[styles.dueLabel, overdue ? styles.dueLabelOverdue : ""].join(" ")}>
          {overdue ? `⚠ Прострочено на ${Math.abs(daysLeft)} дн.` : debt.status === "paid" ? (isReceivable ? "✓ Повернуто" : "✓ Оплачено") : daysLeft === 0 ? "⚡ Сьогодні" : `📅 ${debt.dueDate}${daysLeft > 0 ? ` · залишилось ${daysLeft} дн.` : ""}`}
        </span>
        {debt.type === "installment" && (
          <span className={styles.installInfo}>{debt.paidInstallments}/{debt.totalInstallments} платежів</span>
        )}
        {debt.notes && <span className={styles.notePreview} title={debt.notes}>📝 {debt.notes}</span>}
      </div>
      ...
      <div className={styles.actions}>
        ...
        <div className={styles.actRight}>
          {debt.status !== "paid" && debt.type === "recurring" && (
            <button className={[styles.actBtn, styles.actBtnSecondary].join(" ")} onClick={() => onAddRecurring(debt.id)}>
              + Нарахування
            </button>
          )}
          {debt.status !== "paid" && (
            <button className={[styles.actBtn, styles.actBtnSecondary].join(" ")} onClick={() => onAddPayment(debt.id)}>
              {isReceivable ? "+ Повернення" : "+ Платіж"}
            </button>
          )}
          {debt.status !== "paid" && (
            <button className={[styles.actBtn, styles.actBtnPaid].join(" ")} onClick={() => onMarkPaid(debt.id)}>
              {isReceivable ? "✓ Повернуто" : "✓ Оплачено"}
            </button>
          )}
          <button className={styles.actIconBtn} onClick={() => onEdit(debt.id)} title="Редагувати">✏</button>
          <button className={[styles.actIconBtn, styles.actIconDanger].join(" ")} onClick={() => onDelete(debt.id)} title="Видалити">🗑</button>
        </div>
      </div>
      ...
      {expanded && (
        <div className={styles.expanded}>
          ...
          {debt.paymentHistory?.length > 0 && (
            <div className={styles.section}>
              <div className={styles.sectionTitle}>{isReceivable ? "Історія повернень" : "Історія сплат"}</div>
          ...
          {debt.paymentHistory?.length === 0 && debt.type !== "installment" && (
            <div className={styles.emptyHistory}>{isReceivable ? "Повернень ще не було" : "Платежів ще не було"}</div>
          )}
        </div>
      )}
    </div>
