...
export default function DebtCard({ debt, onEdit, onDelete, onMarkPaid, onAddPayment, onAddRecurring }) {
  ...
  const sc = STATUS_CFG[debt.status] || STATUS_CFG.unpaid;
  ...
  return (
    <div className={[styles.card, styles[`status_${sc.cls}`]].join(" ")}>
      {/* ── Top row ── */}
      ...
      {(debt.type === "installment" || debt.status === "partial") && debt.amount > 0 && (
        <div className={styles.progressWrap}>
          ...
          <span className={styles.progressLabel}>{Math.round(progress)}% сплачено</span>
        </div>
      )}
      ...
      <div className={styles.dueRow}>
        <span className={[styles.dueLabel, overdue ? styles.dueLabelOverdue : ""].join(" ")}>
          {overdue ? `⚠ Прострочено на ${Math.abs(daysLeft)} дн.` : debt.status === "paid" ? "✓ Оплачено" : daysLeft === 0 ? "⚡ Сьогодні" : `📅 ${debt.dueDate}${daysLeft > 0 ? ` · залишилось ${daysLeft} дн.` : ""}`}
        </span>
      ...
      {expanded && (
        <div className={styles.expanded}>
          ...
          {debt.paymentHistory?.length > 0 && (
            <div className={styles.section}>
              <div className={styles.sectionTitle}>Історія сплат</div>
          ...
          {debt.paymentHistory?.length === 0 && debt.type !== "installment" && (
            <div className={styles.emptyHistory}>Платежів ще не було</div>
          )}
        </div>
      )}
    </div>
